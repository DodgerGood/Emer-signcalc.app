...
export default function QuotesPage() {
  ...
  const handleCreate = async (e) => {
    ...
    if (!formData.due_date) {
      toast.error('Please select a job due date');
      return;
    }
              ...
              <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto overscroll-contain touch-pan-y">
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    {newEstimateDueWarning && (
                      <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800 md:col-span-2">
                        Warning: this due date is {newEstimateDueWorkingDays < 0 ? 'overdue' : `${newEstimateDueWorkingDays} working days away`}. It is less than the 15 working day planning window.
                      </div>
                    )}<div className="space-y-2">
                      <Label>Phone</Label>
                      <Input
